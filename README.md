...
- You want to be able to easily add "business logic" to apply, combine, or transform config values. Eg - you might want to convert some database connection parameters into an actual connection instance.

- You need to be able to selectively override config values in fine-grained, complex ways. PowerConfig has robust support for arbitrary environments (`test`, `dev`, `prod`, etc), and also makes it possible to temporarily change values and "lock" them to the file system so that they get picked up by other processes.
...
```python
from simpleconfig import PowerConfig
